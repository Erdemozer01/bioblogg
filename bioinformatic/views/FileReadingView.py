...
def alignment_mapping(request):
    ...
    if BioinformaticModel.objects.filter(user=request.user, tool="AlignmentMapping").exists():
        BioinformaticModel.objects.filter(user=request.user, tool="AlignmentMapping").delete()

    form = MultipleAlignmentForm(request.POST or None, request.FILES or None)

    if request.method == 'POST':
        if form.is_valid():
            file = form.cleaned_data['file']

            obj = BioinformaticModel(
                user=request.user,
                tool="AlignmentMapping"
            )
            ...
            data = open(file_obj.file.path, "r").read()
                                        ...
                                        [
                                            ...
                                            dbc.Tabs(
                                                ...
                                                children=[
                                                    ...
                                                    dcc.Tab(
                                                        label='Alignment Harita',
                                                        children=html.Div(
                                                            className='control-tab mt-2',
                                                            children=[

                                                                html.Label("Dizileme Aracı",
                                                                ...
                                                                dcc.Dropdown(
                                                                    ...
                                                                    options=[
                                                                        ...
                                                                        {'label': 'CLUSTALW2', 'value': 'clustalw2'},
                                                                        {'label': 'Omega', 'value': 'omega'},
                                                                        {'label': 'Paml', 'value': 'paml'},
                                                                    ],
                                                    ...
                                                    dcc.Tab(
                                                        ...
                                                        children=[
                                                            html.Label("Filogenetik ağaç algoritması",
                                                                       className="fw-bolder mt-2"),
            ...
            def update_layout(
                    overview_value, color_value, consensus, gap,
                    conservation, label, show_id, colorscale
            ):
            ...
            @app.callback(
                Output('using-tools-data', 'data'),
                Output('cytoscape-phylogeny', 'elements'),
                Output('cytoscape-phylogeny', 'stylesheet'),
                Output('cytoscape-phylogeny', 'layout'),
                Output('alignment-chart-name', 'children'),

                Input('align-tools', 'value'),
                Input('tree-tools', 'value'),
            )
            def update_tools(tools, tree_alg):

                chart_name = f"{tools} aracılığıyla -{tree_alg} ağacı oluşturuldu".upper()

                global muscle_cline, clustalw2_exe, clustal_omega_exe

                if tools:

                    out_file = obj.records_files.create(
                        file=os.path.join(BASE_DIR, "media", "laboratory", f'{request.user}',
                                          f"{request.user}_alignment.fasta"))

                    xml_tree_file = obj.records_files.create(
                        file=os.path.join(BASE_DIR, "media", "laboratory", f'{request.user}',
                                          f"{request.user}_tree.xml"))
                    ...
                    if tools == 'muscle':
                        ...
                        if sys.platform.startswith('win32'):
                            ...
                            muscle_result = subprocess.check_output(
                                [muscle_exe, "-in", file_obj.file.path, "-out", out_file.file.path])
                    ...
                    aligned_data = open(out_file.file.path, 'r', encoding="utf-8").read()
                    ...
                    return aligned_data, elements, stylesheet, layout, chart_name
